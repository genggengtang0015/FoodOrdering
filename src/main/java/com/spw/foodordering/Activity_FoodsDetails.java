...
public class Activity_FoodsDetails extends BaseActivity implements View.OnClickListener {
    ...
    private TextView tbBack; // 返回
    private TextView tv_price, tv_count, tvMinus, tvAdd,toolbarText;
    ...
    private String tabId; // tab编号
//    private List<Foods> foodsListById = new ArrayList<>();
    private List<FoodCart> foodsListById = new ArrayList<>();
...
//        getFoodsListById(foodId);
    ...
    private void initView() {
        ...
        if (actionBar != null) {
            actionBar.setDisplayHomeAsUpEnabled(false); // 隐藏返回箭头
        }
...
//                return true;
     ...
     * @param foodId
     */
    private void getFoodsListByTabId(final String tabId) {
        pg.setMessage("加载中...");
        pg.show();
            ...
            public void run() {
                try {
                    if(tabId != null && !tabId.isEmpty()){
                        foodsListById = RequestUtility.getMainFoods(Util.SHOP_ID,tabId);
                    }else{
                        foodsListById = RequestUtility.getMainFoods(Util.SHOP_ID,"1");
                    }
...
//            Foods foods = foodsListById.get(0);
            FoodCart foods = foodsListById.get(0);
            imageUrl = foods.getImageUrl();
            name = foods.getFoodName();
            price = foods.getPriceView();
//            imageUrl = foods.getImageUrl();
//            name = foods.getName();
