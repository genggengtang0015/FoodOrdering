...
public class RequestTask extends AsyncTask<Request, Void, String> {
     ...
     * @param params 请求所需的参数
     */
    public void requestPOSTWithHeaders(String url, Map<String, String> headers, Map<String, String> params) {
        FormBody.Builder builder = new FormBody.Builder();
//        for (int i = 0; i < params.length; i += 2) {
//            builder.add(params[i], params[i + 1]);
//        }
        ...
        Request request = rBuilder.post(builder.build()).url(url).build();
        this.execute(request);
    }
